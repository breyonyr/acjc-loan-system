  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-start justify-between">
        ...
        <Link
          href="/borrow"
          className="inline-flex h-9 items-center justify-center rounded-md bg-foreground px-3.5 text-sm font-medium text-background transition-colors hover:bg-foreground/90"
        >
          Borrow equipment
        </Link>
      </div>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          ...
          {activeCount > 0 && (
            <span className="rounded-full bg-foreground/5 px-2 py-0.5 text-xs font-medium text-muted-foreground">{activeCount}</span>
          )}
        </div>
        {batchGroups.length > 0 ? (
          <div className="flex flex-col gap-2">
            {batchGroups.map((group) => {
              ...
              if (isBatch) {
                ...
                const borrowDate = group.loans[0].checked_out_at;
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border p-4 ${
                      hasOverdue ? "border-red-200 bg-red-50/30" : "border-border bg-card"
                    }`}
                  >
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium">
                          ...
                          <span className="text-muted-foreground font-normal">
                            {" "}&middot; Borrowed {format(new Date(borrowDate), "MMM d")}
                          </span>
                        </p>
                      </div>
                      <ReturnBatchButton
                        ...
                        return (
                          ...
                          >
                            <div className="min-w-0 flex-1">
                              ...
                              <p className={`mt-0.5 text-xs font-medium ${isOverdue ? "text-red-600" : "text-muted-foreground"}`}>
                                {isOverdue
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Borrowed {format(new Date(loan.checked_out_at), "MMM d")}
                    </p>
                    <p className={`mt-0.5 text-xs font-medium ${isOverdue ? "text-red-600" : "text-muted-foreground"}`}>
                      {isOverdue
                        ? `Overdue by ${formatDistanceToNow(dueDate)}`
          ...
          <div className="rounded-lg border border-dashed border-border px-6 py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              No active loans.{" "}
              <Link href="/borrow" className="font-medium text-foreground underline underline-offset-4 hover:no-underline">
                Borrow equipment
              </Link>{" "}
              to get started.
            </p>
